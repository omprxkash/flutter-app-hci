...
void main() {
  ...
  group('PHQ-9 scoring', () {
    ...
    test('all "Not at all" -> 0 (minimal)', () {
      final List<Answer> answers = phq9.questions
          .map(
            (q) => Answer(questionId: q.id, selectedOptionIds: <String>['0']),
          )
          .toList();
    ...
    test('all "Nearly every day" -> 27 (severe)', () {
      final List<Answer> answers = phq9.questions
          .map(
            (q) => Answer(questionId: q.id, selectedOptionIds: <String>['3']),
          )
          .toList();
      final ScoreResult r = score(phq9, answers);
      expect(r.score, 27);
  ...
  group('GAD-7 scoring', () {
    ...
    test('score 7 -> mild band', () {
      ...
      final List<Answer> answers = <Answer>[
        Answer(
          questionId: gad7.questions[0].id,
          selectedOptionIds: const <String>['3'],
        ),
        Answer(
          questionId: gad7.questions[1].id,
          selectedOptionIds: const <String>['2'],
        ),
        Answer(
          questionId: gad7.questions[2].id,
          selectedOptionIds: const <String>['2'],
        ),
        Answer(
          questionId: gad7.questions[3].id,
          selectedOptionIds: const <String>['0'],
        ),
        Answer(
          questionId: gad7.questions[4].id,
          selectedOptionIds: const <String>['0'],
        ),
        Answer(
          questionId: gad7.questions[5].id,
          selectedOptionIds: const <String>['0'],
        ),
        Answer(
          questionId: gad7.questions[6].id,
          selectedOptionIds: const <String>['0'],
        ),
      ];
      final ScoreResult r = score(gad7, answers);
  ...
  group('Preset library', () {
    test('contains PHQ-9, GAD-7, and MMSE', () {
      ...
      expect(
        all.map((q) => q.id),
        containsAll(<String>['preset_phq9', 'preset_gad7', 'preset_mmse']),
      );
    });
    ...
    test('all presets cite a reference', () {
      expect(
        PresetQuizzes.all().every(
          (q) => q.reference != null && q.reference!.isNotEmpty,
        ),
        isTrue,
      );
    });
